...
def get_final_dataset(images_and_bboxes, bboxes_per_image):
    ...
    def sample_negatives(img, boxes, cls):
        return {"image": tf.cast(tf.image.random_crop(img, size=[IMAGE_SIZE, IMAGE_SIZE, 3]), np.float32),
                "bbox_id": -1, "label": 0}
    ...
    for shard, (image, label, bbox_id) in enumerate(ds):
        ...
        with tf.io.TFRecordWriter(filename) as out_file:
            for i in range(shard_size):
                print(image.numpy()[i])
                example = to_tfrecord(image.numpy()[i],  # re-compressed image: already a byte string
                                      label.numpy()[i],
                                      bbox_id.numpy()[i])
...
@click.option('--input-bbox-csv', help='Path to output object location CSV', required=True, type=str)
@click.option('--output-tfrecord-path', help='Path to output object location CSV', required=True, type=str)
