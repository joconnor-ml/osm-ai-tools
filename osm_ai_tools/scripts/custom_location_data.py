...
@click.command()
@click.option('--input-csv', help='Must contain at least a lat lon pair and ID for each object', required=True,
              type=str)
...
@click.option('--object-size', 'apprrox object span in lat/lon space', default=0.001, type=float)
@click.option('--object-class', help='Path to output object location CSV', required=True, type=str)
def cli(input_csv, id_col, lat_col, lon_col, object_size, object_class):
    ...
    df[["osm_id","min_lat","max_lat","min_lon","max_lon","center_lat","center_lon","object_class"]].to_csv(input_csv)
